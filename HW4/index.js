...
const AppController = ((view, model) => {
    ...
    const initTodos = () => {
        model.fetchTodos().then(data => {
            ...
            const todoInput = document.getElementsByClassName(view.domString.todoinput);
            todoInput[0].addEventListener('change', (event) => {
                const newTodo = new Model.Todo('',event.target.value,event.target.value,'');
