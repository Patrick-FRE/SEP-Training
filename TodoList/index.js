...
const AppController = ((view, model) => {
    ...
    const initTodos = () => {
        model.fetchTodos().then(data => {
            ...
            const inputEle = document.querySelector('.todolist__input')
            inputEle.addEventListener('keypress', evt =>{
                    if(evt.code=='Enter'){
                        var newTodoInput = document.getElementById('todo_input').value
                        console.log(newTodoInput)
                        var newTodo = new Model.Todo(10,state.todolist.length, newTodoInput, false)
